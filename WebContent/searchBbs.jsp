	...
	<nav class="navbar navbar-default" style="background-color:#2c2c2c; font-color:FFFFFF; font-weight:bold">
		...
		<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
			<ul class="nav navbar-nav">
				...
				<% if (boardID == 1){ %>
					<li class="active"><a href="bbs.jsp?boardID=1">동호회 이야기</a></li>
					<li><a href="bbs.jsp?boardID=2">자유로운 이야기</a></li>
				<%} else if(boardID == 2){ %>
					<li><a href="bbs.jsp?boardID=1">동호회 이야기</a></li>
					<li class="active"><a href="bbs.jsp?boardID=2">자유로운 이야기</a></li>
				<% } %>
			</ul>
			<%
	...
	<% }%>
		<div class="container">
			<div class="row">
				<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
					<thead>
						<tr>
							...
							<th style="background-color: #eeeee; text-align: left;">제목</th>
					...
					<tbody>
						...
						<tr>
							...
							<td style="text-align:left"><a href="view.jsp?boardID=<%=boardID%>&bbsID=<%= list.get(i).getBbsID() %>"><%= list.get(i).getBbsTitle().replaceAll(" ", "&nbsp;").replaceAll("<", "&lt;").replaceAll(">","&gt;").replaceAll("\n","<br>") %></a></td>
				...
				<form name = "p_search">
					<input type="button" class="btn btn-primary" value="검색" onclick="nwindow(<%=boardID%>)"/>
					<a href="write.jsp?boardID=<%=boardID%>" class="btn btn-primary" style="float:right">글쓰기</a>
				</form>	
			</div>
		</div>
	</div>
